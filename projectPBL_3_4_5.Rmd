...
regulators <- interactions %>%
  select(Regulator_systematic, Regulator_standard) %>%
  distinct() %>%
  rename(group = Regulator_systematic, id = Regulator_standard)
targets <- interactions %>%
  select(Target_systematic, Target_standard) %>%
  distinct() %>%
  rename(group = Target_systematic, id = Target_standard)
nodes <- dplyr::union(regulators, targets)
edges <- interactions %>%
  select(Regulator_standard, Target_standard, Regulation) %>%
  rename(source = Regulator_standard, target = Target_standard, weight = Regulation) %>%
  distinct()
...
### Plot for fold changes with DEseq2 

# TODO: do the plot
$\to$ plot everything (facetwrap?) scatter to plot conditions against each other
