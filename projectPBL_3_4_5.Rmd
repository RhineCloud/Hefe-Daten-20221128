...
## DESeq and foldChanges (Huber chapter 8)

For the analysis, we wil analyze the data according to the temperature. For that we had to transfrom the data into the correct format via DESeqDataFromMatrix and could then use the DESeq-method of the package in order to create some interesting plots and get a list of genes and their foldchanges throughout the different conditions.
...
# convert object
deseqData <- DESeqDataSetFromMatrix(countData = countData[,rownames(colData)], colData = colData, ~temperature)
class(deseqData)
...
pheatmap( assay(pas_rlog)[select, ],
     scale = "row", fontsize = 5, main = "Heatmap of regularized log transformed data of the top 30 genes")
```
